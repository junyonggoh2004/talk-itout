...
import { EmotionType, emotionBlendShapes, VisemeType, visemeBlendShapes } from '@/utils/visemeMapping';

// Ready Player Me avatar with morph targets
const AVATAR_URL = 'https://models.readyplayer.me/6409e7b7c5f4c90001c5c5a2.glb?morphTargets=ARKit,Oculus Visemes';
...
const AvatarModel = ({ emotion, audioVolume, isSpeaking, currentViseme }: AvatarModelProps) => {
  ...
  useFrame((_, delta) => {
    ...
    if (isSpeaking && audioVolume > 0.05) {
      // Map current viseme to blend shape
      const visemeIndex = visemeBlendShapes[currentViseme];
      const visemeShapeName = currentViseme;
      
      if (dict[visemeShapeName] !== undefined) {
        const targetInfluence = Math.min(audioVolume * 1.5, 1);
        influences[dict[visemeShapeName]] = THREE.MathUtils.lerp(
          influences[dict[visemeShapeName]],
          targetInfluence,
          lerpSpeed * 2
        );
      }
      ...
      if (dict['jawOpen'] !== undefined) {
        const jawTarget = audioVolume * 0.4;
